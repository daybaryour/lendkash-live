            ...
            <tbody>
                ...
                <tr>
                    <td valign="top" style="border:1px solid #2A05A7; padding:15px">
                        ...
                        @if($data['type']=='investApprove')
                            <p>
                                Your invest Request Id {{$data['requestId']}} is {{$data['status']}} by admin.
                                @if($data['status']=='approved')
                                    Your amount Naira {{$data['invest_amount']}} is deducted from your wallet.
                                @endif
                            </p>
                        @endif
                        @if($data['type']=='investMaturity')
                            <p>
                                Your invest Request Id {{$data['requestId']}} is matured.
                                Maturity amount of Naira {{$data['maturity_amount']}} for invest Request ID {{$data['requestId']}} is received in your wallet.
                            </p>
                        @endif
